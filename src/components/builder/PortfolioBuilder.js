...
const PortfolioBuilder = () => {
  ...
  const fetchBonds = async () => {
    ...
    try {
      ...
      setStep(7); // Paso siguiente: disruptivas
    } catch (err) {
  ...
  // Paso 6: Buscar Disruptivas
  const fetchDisruptivas = async () => {
    setLoading(true); setError('');
    try {
      const amount = parseFloat(formData.amount) * (parseInt(formData.allocation.disruptivas) / 100);
      ...
      if (!response.ok) throw new Error(await response.text());
      setPortfolioDisruptivas(await response.json());
    } catch (err) {
  ...
  useEffect(() => {
    if (step === 6) {
      ...
      if (portfolioDisruptivas && portfolioDisruptivas.allocation) allocation.disruptivas = portfolioDisruptivas.allocation;
      const final = {
        allocation,
        metrics: {},
        source: 'combinado',
    ...
    <div className="portfolio-builder">
      ...
      {step === 2 && (
        <div className="step-container">
          ...
          <label>Disruptivas %: <input type="number" value={formData.allocation.disruptivas} onChange={e => handleAllocationChange('disruptivas', e.target.value)} /></label>
      ...
      {step === 5 && (
        <div className="step-container">
          ...
          {portfolioBonds && (
            <div>
              <button onClick={() => setStep(7)} className="optimize-button">Buscar Disruptivas</button>
            </div>
          )}
          {error && <div className="error-message">{error}</div>}
        </div>
      ...
      {step === 7 && (
        <div className="step-container">
          <h2>Paso 6: Buscar Disruptivas</h2>
          <button onClick={fetchDisruptivas} disabled={loading} className="optimize-button">
