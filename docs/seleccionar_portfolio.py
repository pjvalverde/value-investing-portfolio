    ...
    for datos in datos_completos:
        if datos.get("ticker") == ticker and "error" not in datos:
            try:
                ...
                chart_data = datos.get("chart_data", {})
                result = chart_data.get("chart", {}).get("result", [{}])[0]
                
                # Extraer precios históricos si están disponibles
                timestamps = result.get("timestamp", [])
                precios = result.get("indicators", {}).get("quote", [{}])[0].get("close", [])
                
                # Calcular tendencia de precios (últimos 12 meses vs anteriores)
                if len(precios) >= 24:  # Al menos 2 años de datos
                    precio_reciente = np.mean(precios[-12:])  # Último año
                    precio_anterior = np.mean(precios[-24:-12])  # Año anterior
                    tendencia_precio = (precio_reciente / precio_anterior - 1) * 100  # % cambio
                else:
                    tendencia_precio = np.nan
                
                # Extraer eventos significativos
                insights = datos.get("insights", {})
                result_insights = insights.get("finance", {}).get("result", {})
                
                # Extraer desarrollos significativos
                sig_devs = result_insights.get("sigDevs", [])
                num_sig_devs = len(sig_devs)
                
                # Extraer informes SEC
                sec_reports = result_insights.get("secReports", [])
                num_sec_reports = len(sec_reports)
                
                return {
                    "tendencia_precio": tendencia_precio,
                    "num_sig_devs": num_sig_devs,
                    "num_sec_reports": num_sec_reports
                }
            except Exception as e:
                print(f"Error al extraer métricas adicionales para {ticker}: {str(e)}")
                return {}
    
    return {}

# Aplicar filtros cuantitativos según criterios de value investing
print("Aplicando filtros cuantitativos...")

# 1. Filtrar empresas con valoración favorable
df_filtrado = df_combinado[
    (df_combinado["Valoración"].str.contains("Undervalued", na=False, case=False)) |
    (df_combinado["Valoración"].str.contains("Fair", na=False, case=False))
].copy()
